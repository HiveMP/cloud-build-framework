...
namespace UnrealBuildTool
{
	class FASTBuild : ActionExecutor
	{
		...
		private eCacheMode CacheMode = eCacheMode.ReadOnly;
		private string CachePath = Environment.GetEnvironmentVariable("FASTBUILD_CACHE_PATH");
		private static string FBuildExePathOverride = Environment.GetEnvironmentVariable("FASTBUILD_FBUILD_PATH");
		...
		public FASTBuild()
		{
			var unrealLaunchDashboard = Environment.GetEnvironmentVariable("FASTBUILD_UNREAL_LAUNCH_DASHBOARD") == "True";
			var fbDashboardPath = Environment.GetEnvironmentVariable("FASTBUILD_DASHBOARD_PATH");
			var enableCaching = Environment.GetEnvironmentVariable("FASTBUILD_UNREAL_ENABLE_CACHE") == "True";
			var enableCacheWriting = Environment.GetEnvironmentVariable("FASTBUILD_UNREAL_WRITE_CACHE") == "True";
			var enableDistributed = Environment.GetEnvironmentVariable("FASTBUILD_UNREAL_ENABLE_DISTRIBUTED");

			bEnableCaching = enableCaching;
			CacheMode = enableCacheWriting ? eCacheMode.ReadWrite : eCacheMode.ReadOnly;
			
			if (unrealLaunchDashboard)
			{
				if (Process.GetProcessesByName("FBDashboard").Length == 0)
				{
					var info = new ProcessStartInfo
					{
						FileName = fbDashboardPath,
						WorkingDirectory = new FileInfo(fbDashboardPath).DirectoryName,
					};
					Process.Start(info);
				}
			}

			if (enableDistributed == "True")
			{
				bEnableDistribution = true;
			}
			else if (enableDistributed == "Check")
			{
				var distributionTarget = Environment.GetEnvironmentVariable("FASTBUILD_UNREAL_DISTRIBUTED_TARGET");
				var ping = new Ping();
				var reply = ping.Send(distributionTarget, 100);
				bEnableDistribution = reply.Status == IPStatus.Success;
			}
			else
			{
				bEnableDistribution = false;
			}

			Console.WriteLine("FASTBuild Path: " + FBuildExePathOverride);
			Console.WriteLine("FASTBuild Dashboard Path: " + fbDashboardPath);
			Console.WriteLine("FASTBuild Cache Enabled: " + bEnableCaching.ToString());
			Console.WriteLine("FASTBuild Cache Mode: " + CacheMode.ToString());
			Console.WriteLine("FASTBuild Distribute Enabled: " + bEnableDistribution.ToString());
		}
		...
		private bool ExecuteBffFile(string BffFilePath)
		{
			...
			Console.WriteLine("FASTBuild Argument List: " + FBCommandLine);
